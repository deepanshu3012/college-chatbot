...
def rebuild_knowledge_base(pdf_path: str):
    ...
    # ── Delete all existing docs and re-add ──
    try:
    ...
    vectordb.persist()
    retriever = vectordb.as_retriever(search_kwargs={"k": 6})
    print(f"✅ Knowledge base rebuilt with {len(unique_chunks)} unique chunks")
...
async def admin_panel(pwd: str = ""):
    ...
    pdf_exists = os.path.exists("data/college_info.pdf")
    pdf_status = "✅ college_info.pdf found" if pdf_exists else "⚠️ No PDF found in data/"
    db_exists = os.path.exists("./db") and len(os.listdir("./db")) > 0
    db_status = "✅ Vector database ready" if db_exists else "⚠️ Database not built"
...
<style>
  *{{box-sizing:border-box;margin:0;padding:0}}
  ...
  .hlink{{background:rgba(74,222,128,.15);border:1px solid rgba(74,222,128,.3);border-radius:8px;color:#4ade80;font-size:.72rem;font-weight:700;padding:.35rem .8rem;text-decoration:none;transition:all .2s}}
...
<div class="header">
  ...
  <div class="header-links">
    <a href="/dashboard?pwd={pwd}" class="hlink">📊 Dashboard</a>
...
<div class="section">
  ...
  <div class="status-row">
    <div class="status-badge">{pdf_status}</div>
    <div class="status-badge">{db_status}</div>
  </div>
  <div class="upload-form">
    <p style="font-size:.82rem;color:#6b7280;margin-bottom:.5rem">Upload a new college PDF to update the knowledge base. This will automatically rebuild the vector database.</p>
    <form method="POST" action="/admin/upload?pwd={pwd}" enctype="multipart/form-data">
  <input type="file" name="file" accept=".pdf" required style="border:2px dashed #bbf7d0;border-radius:12px;padding:1rem;width:100%;font-size:.85rem;color:#14532d;background:#f0fdf4;cursor:pointer;margin-bottom:.5rem"/>
  <div class="btn-row" style="margin-top:.5rem">
    <button type="submit" class="btn btn-green">📤 Upload & Rebuild Knowledge Base</button>
  </div>
</form>
    <div class="alert" id="upload-alert">✅ PDF uploaded and knowledge base rebuilt successfully!</div>
  </div>
</div>
...
<style>
  *{{box-sizing:border-box;margin:0;padding:0}}body{{background:#f0faf4;font-family:'Plus Jakarta Sans',sans-serif;padding:2rem 1rem}}
  ...
  .section{{background:white;border:1.5px solid #bbf7d0;border-radius:16px;padding:1.4rem;margin-bottom:1.5rem}}
  .section h2{{font-size:.85rem;font-weight:700;color:#16a34a;text-transform:uppercase;letter-spacing:.08em;margin-bottom:1rem}}
  table{{width:100%;border-collapse:collapse;font-size:.85rem}}th{{background:#f0fdf4;color:#16a34a;font-weight:700;padding:.6rem .8rem;text-align:left;font-size:.75rem;text-transform:uppercase}}
